...
export default function EditTenant() {
    ...
    useEffect(() => {
        if (user && !hasPermission('locataires', 'edit')) {
            navigate('/tenants');
            Swal.fire('Accès refusé', 'Vous n\'avez pas la permission de modifier un locataire.', 'error');
        }
    ...
    const loadTenantData = async () => {
        try {
            ...
            if (response.success) {
                ...
                setFormData({
                    prenom: tenant.user?.prenom || '',
                    nom: tenant.user?.nom || '',
                    email: tenant.user?.email || '',
                    telephone: tenant.user?.telephone || '',
                    numero_cni: tenant.numero_cni || '',
                    date_naissance: tenant.date_naissance || '',
                    lieu_naissance: tenant.lieu_naissance || ''
                });
                setExistingCniRecto(tenant.cni_recto);
                setExistingCniVerso(tenant.cni_verso);
            }
        } catch (error) {
            console.error(error);
            Swal.fire('Erreur', 'Impossible de charger les données du locataire', 'error');
            navigate('/tenants');
        } finally {
        ...
        try {
            ...
            if (response.success) {
                Swal.fire({
                    icon: 'success',
                    title: 'Modifié !',
                    text: 'Le locataire a été modifié avec succès.',
                    timer: 2000,
    ...
    return (
        <div className="max-w-2xl mx-auto">
            <h1 className="text-2xl font-bold text-gray-900 mb-6">Modifier le Locataire</h1>
